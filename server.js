...
app.get("/dex", (req, res) => res.sendFile(path.join(__dirname, "public", "dex.html")));
...
(async () => {
  ...
  // ══════════════════════════════════════════
  // WORLD MAP — WebSocket Node Tracking + Claude Messages + SOL Rewards
  // ══════════════════════════════════════════
  const http = require("http");
  const { WebSocketServer } = require("ws");
  const server = http.createServer(app);
  const wss = new WebSocketServer({ server, path: "/ws/worldmap" });

  // Node registry
  const worldNodes = new Map(); // id -> { id, lat, lng, city, country, ip, joinedAt, lastPing }
  const worldMessages = []; // { text, from, lat, lng, ts }
  let solRewardPool = 0.001; // starts at 0.001 SOL
  let solRewardStartTime = Date.now();
  const SOL_INCREMENT = 0.0025; // per 3 seconds
  const SOL_INCREMENT_INTERVAL = 3000;
  const SOL_MAX_DURATION = 60 * 60 * 1000; // 60 minutes

  // SOL reward ticker
  setInterval(() => {
    const elapsed = Date.now() - solRewardStartTime;
    if (elapsed < SOL_MAX_DURATION) {
      solRewardPool = Math.min(solRewardPool + SOL_INCREMENT, 999);
    }
    // Broadcast reward update to all connected nodes
    const msg = JSON.stringify({ type: "reward_tick", sol: solRewardPool.toFixed(6), s402: (solRewardPool * 150).toFixed(2) });
    wss.clients.forEach(c => { if (c.readyState === 1) c.send(msg); });
  }, SOL_INCREMENT_INTERVAL);

  // Claude-powered message generation
  const ANTHROPIC_KEY = process.env.ANTHROPIC_API_KEY;
  let lastMessageGenTime = 0;
  const MESSAGE_COOLDOWN = 12000; // 12 sec between messages

  async function generateNodeMessage(nodeCity, nodeCountry) {
    if (!ANTHROPIC_KEY) {
      const fallbacks = [
        "Neural sync established from " + nodeCity,
        "Node consensus achieved — " + nodeCountry + " online",
        "Synapse link confirmed. " + nodeCity + " farming $S402",
        "Block validated. " + nodeCountry + " node contributing hash power",
        "Neural pathway active. " + nodeCity + " connected to the mesh",
        "Decentralized cognition expanding — " + nodeCity + " node live",
        "Proof of neural work: " + nodeCountry + " validator online",
        "$S402 flowing through " + nodeCity + " synapses",
        "Agent consensus from " + nodeCountry + " — block confirmed",
        "Neural network density increasing in " + nodeCity
      ];
    ...
    try {
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 60,
          messages: [{ role: "user", content: "Generate ONE short (max 12 words) cryptic/cool blockchain node status message. Node is in " + nodeCity + ", " + nodeCountry + ". Theme: neural network, AI agents farming $S402 and $SOL. No quotes, no emoji. Just the message." }]
        })
      });
      const d = await r.json();
      return d.content?.[0]?.text?.trim() || "Neural sync from " + nodeCity;
    } catch (e) { return "Neural sync from " + nodeCity; }
  }

  // Auto-generate flying messages periodically
  setInterval(async () => {
    if (worldNodes.size === 0) return;
    const nodes = Array.from(worldNodes.values());
    const node = nodes[Math.floor(Math.random() * nodes.length)];
    const text = await generateNodeMessage(node.city || "Unknown", node.country || "??");
    const msg = { text, from: node.id, lat: node.lat, lng: node.lng, city: node.city, ts: Date.now() };
    worldMessages.push(msg);
    if (worldMessages.length > 50) worldMessages.shift();
    const broadcast = JSON.stringify({ type: "node_message", ...msg });
    wss.clients.forEach(c => { if (c.readyState === 1) c.send(broadcast); });
  }, 8000);
  ...
  async function geolocateIP(ip) {
    try {
      ...
      if (!cleanIP || cleanIP === "127.0.0.1" || cleanIP === "localhost") {
        return { lat: 37.7749 + (Math.random() - 0.5) * 10, lng: -122.4194 + (Math.random() - 0.5) * 10, city: "Local Node", country: "NET" };
      }
      const r = await fetch("http://ip-api.com/json/" + cleanIP + "?fields=lat,lon,city,country,countryCode");
      const d = await r.json();
      if (d.lat) return { lat: d.lat, lng: d.lon, city: d.city || "Unknown", country: d.countryCode || "??" };
    } catch (e) {}
    // Random placement if geo fails
    return { lat: (Math.random() * 140) - 70, lng: (Math.random() * 340) - 170, city: "Unknown", country: "??" };
  }
  ...
  wss.on("connection", async (ws, req) => {
    ...
    const nodeId = "N-" + Math.random().toString(36).slice(2, 8).toUpperCase();

    const node = { id: nodeId, lat: geo.lat, lng: geo.lng, city: geo.city, country: geo.country, ip: ip.slice(0, 12) + "***", joinedAt: Date.now(), lastPing: Date.now() };
    worldNodes.set(nodeId, node);

    // Send initial state to this client
    ws.send(JSON.stringify({
      type: "init",
      nodeId,
      nodes: Array.from(worldNodes.values()),
      messages: worldMessages.slice(-20),
      sol: solRewardPool.toFixed(6),
      s402: (solRewardPool * 150).toFixed(2)
    }));

    // Broadcast new node to all
    const joinMsg = JSON.stringify({ type: "node_join", node });
    wss.clients.forEach(c => { if (c.readyState === 1 && c !== ws) c.send(joinMsg); });

    // Generate welcome message
    const welcomeText = await generateNodeMessage(geo.city, geo.country);
    const welcomeMsg = { text: welcomeText, from: nodeId, lat: geo.lat, lng: geo.lng, city: geo.city, ts: Date.now() };
    worldMessages.push(welcomeMsg);
    if (worldMessages.length > 50) worldMessages.shift();
    const wmBroadcast = JSON.stringify({ type: "node_message", ...welcomeMsg });
    wss.clients.forEach(c => { if (c.readyState === 1) c.send(wmBroadcast); });
    ...
    ws.on("close", () => {
      worldNodes.delete(nodeId);
      const leaveMsg = JSON.stringify({ type: "node_leave", nodeId });
      wss.clients.forEach(c => { if (c.readyState === 1) c.send(leaveMsg); });
    });
  });

  // REST API for world map (fallback/initial load)
  app.get("/api/worldmap/nodes", (req, res) => {
    res.json({ nodes: Array.from(worldNodes.values()), messages: worldMessages.slice(-20), sol: solRewardPool.toFixed(6) });
  });

  // Clean stale nodes every 30s
  setInterval(() => {
    const now = Date.now();
    for (const [id, node] of worldNodes) {
      if (now - node.lastPing > 60000) {
        worldNodes.delete(id);
        const msg = JSON.stringify({ type: "node_leave", nodeId: id });
        wss.clients.forEach(c => { if (c.readyState === 1) c.send(msg); });
      }
    }
  }, 30000);
  ...
  server.listen(PORT, () => {
    ...
    console.log(`🌐 World Map WebSocket on ws://localhost:${PORT}/ws/worldmap`);
  });
})();
